...
function createDateUtc(params) {
    ...
    if (params === 'UTC') {
        h3.textContent = `${now.toLocaleString('en-US', options)} à Iceland/Reykjavik `;
    }else {
...
let h3 = document.createElement('h3');
body.appendChild(h3);
h3.textContent = myBirthday.getTime() / (1000 * 60 * 60 * 24) + ' Day after my birthday';
...
function dayAfterThisDate(year, month, day,) {
    ...
    let h3 = document.createElement('h3');
    body.appendChild(h3);
    h3.textContent = calc + ' Day after this date ' + year + ' ' + month+  ' ' + day;
}
